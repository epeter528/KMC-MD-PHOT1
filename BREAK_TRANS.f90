subroutine BREAK_TRANSLATIONAL_MOTION(aminoresnum,number_of_scans,expconst,resnumber,scanbool,energy_scan,g_bool,maxi,modulonumber,break)
...
real(kind=8),dimension(1000)           :: maximum

integer,dimension(1000)                :: hbmap_donor,hbmap_hydrogen,hbmap_acceptor

character(len=23)                     :: readchar

logical                               :: ex,new_break

real(kind=8)                          :: acc_x,acc_y,acc_z, target_x,target_y,target_z
...
real(kind=8),dimension(10000)           :: NEW_X,NEW_Y,NEW_Z

real(kind=8),dimension(10000)           :: don_x_res,don_y_res,don_z_res
...
do k=1,natoms
           ...
           read(87,'(i5,2a5,i5,3f8.3,3f8.4)',end=9010,err=9010) resnum(k), restype(k), atomtype(k), atomnum(k), coord_x(k), &
...
open(unit=1,file='rms.sh')

write(1,*) '#!/bin/bash'
write(1,'(a91)') '/loctmp/pee18323/GRO_bin/bin/g_rmsf -f md_traj.trr -s md.tpr -n 2.ndx -dir rmsf.log << EOF'
write(1,*) '1' 
write(1,*) 'EOF'

close(unit=1)

call system('chmod 744 rms.sh')

call system('./rms.sh')

open(unit=1,file='rmsf.log')
...
enddo
 ...
 read(1,'(a10,f7.4)') char10,target_x
 read(1,'(a10,f7.4)') char10,target_y
 read(1,'(a10,f7.4)') char10,target_z
...
call system ('rm 4.ndx')

call system ('cat 1.ndx 3.ndx > 4.ndx')
...
write(1,*) '#!/bin/bash'
write(1,'(a91)') '/loctmp/pee18323/GRO_bin/bin/g_rmsf -f md_traj.trr -s md.tpr -n 4.ndx -dir rmsf.log << EOF'
write(1,*) '1' 
write(1,*) 'EOF'
...
sc = 0
   ...
   do n=1,number_of_scans
                  ...
                  new_x(p) = sc*(target_x)+don_x_res(p) 
                  new_y(p) = sc*(target_y)+don_y_res(p) 
                  new_z(p) = sc*(target_z)+don_z_res(p)
...
l = 1
       ...
       call scan_energy_of_protein(energy_val,alert)
