...
integer                :: scansteps,scansteps2

real(kind=8)           :: min_delta_g,max_delta_g
...
real(kind=8)           :: maxtime,time_end,rate,total_time

integer                :: md_time

integer                :: select,scannumber

integer                :: step,maxi,modulonumber
...
call noe_sep(noe_max)

open(unit=144,file='index2.sh')

write(144,*) '#!/bin/bash'

write(144,'(a76)') '/loctmp/pee18323/GRO_bin/bin/make_ndx -f minimized3.gro -o system.ndx << EOF'
...
call BREAK_TRANSLATIONAL_MOTION(aminoresnum,scansteps,noe_val(i)*max_rmsd1,i,scanbool,energy_scan,g_bool,maxi,modulonumber)

open(unit=1,file='H_search_new2_int.sh')

write(1,*) '#!/bin/bash'
write(1,'(a96,f4.2,a7)') '/loctmp/pee18323/GRO_bin/bin/g_hbond -f start.gro -n 2.ndx -s run.tpr -hbn table_hbond_B.ndx -r ', 0.35+noe_val(i)*max_rmsd1,' << EOF'
write(1,*) '1'
write(1,*) '2'
write(1,*) 'EOF'
...
write(*,*) 'selected break trans',selectamino
           ...
           call BREAK_TRANSLATIONAL_MOTION(aminoresnum,scansteps2,expconst2,i,scanbool,energy_scan,g_bool,maxi,modulonumber)
